...
const Footer = () => {
  ...
  return (
    <footer className="container mx-auto divide-y py-12 px-4 ">
      <div className="flex flex-col pb-10 lg:flex-row">
        ...
        <div className="mt-10 grid grid-cols-2 gap-10 md:grid-cols-4">
          {footerColumns.map((footerColumn, index) => (
            <div
              key={index}
              className={`${footerColumn.links ? 'col-span-1' : 'col-span-2'}`}
            >
              <h3 className="mb-4 font-semibold text-slate-600">
                {footerColumn.category}
              </h3>
              {footerColumn.links && (
                <ul>
                  {footerColumn.links.map((link, linkIndex) => (
                    <li key={linkIndex} className="my-2  first:my-0">
                      <Link href={link.url}>
                        <a className="h-6 border-gray-500 hover:border-b-[1px]">
                          {link.name}
                        </a>
                      </Link>
                    </li>
              ...
              {footerColumn.description && (
                <p className="my-6 text-gray-800">{footerColumn.description}</p>
              )}
              {footerColumn.CTAButton && (
                <Button
                  variant="light"
                  className="my-4 flex w-fit gap-2"
                  href={footerColumn.CTAButton.url}
                >
                  {footerColumn.CTAButton.icon &&
                    footerColumn.CTAButton.icon.isHeroIcon && (
                      <Icon
                        className="h-5 w-5"
                        icon={footerColumn.CTAButton.icon.url as IconName}
                      />
                    )}
      ...
      <div className="pt-10 text-center text-sm text-gray-800 md:text-left">
        © 2022 Registered by CRR
      </div>
    </footer>
  )
}
